...
awesome.spawn("conky -c" .. os.getenv("HOME") .. "/.config/conky/blue.conkyrc")

local theme                                     = {}
theme.dir                                       = os.getenv("HOME") .. "/.config/awesome/themes/dt-like-blue"
local iconsdir                                  = os.getenv("HOME") .. "/.config/awesome/themes/"
theme.wallpaper                                 = theme.dir .. "/wallpaper.jpg"
-- theme.font                                      = "Noto Sans Regular 11"
theme.font                                      = "Ubuntu Bold 10"
theme.layoutfont                                = "Ubuntu Bold 8"
...
local bar_widget_sep_margin_left = 0
local bar_widget_sep_margin_right = 0
...
local brightness = awful.widget.watch(
    ...
    function(widget, stdout)
        widget:set_markup(" " .. markup.fontfg(theme.font, "#689dcd", stdout))
    end
)
-- local layoutname = awful.widget.watch(
--     'awesome-client \'return mouse.screen.selected_tag.layout.name\' | cut -d \'"\' -f 2', 1,
--     function(widget, stdout)
--         widget:set_markup(" " .. markup.fontfg(theme.font, "#467b96", stdout))
--     end
-- )
...
local layoutname = awful.widget.watch(
    ...
    function(widget, stdout)
        widget:set_markup(" " .. markup.fontfg(theme.layoutfont, "#dfdfdf", stdout))
    end
)
-- Separators
...
local mainSep1 = awful.widget.watch(
    "echo '◢' ", 60,
    function(widget, stdout)
        widget:set_markup(" " .. markup.fontfg("Ubuntu Mono 50", "#689dcd", stdout))
    end
)

local mainSep2 = awful.widget.watch(
    "echo '◢' ", 60,
    function(widget, stdout)
        widget:set_markup(" " .. markup.fontfg("Ubuntu Mono 50", "#889fa7", stdout))
    end
)
...
local mem = lain.widget.mem({
    settings = function()
        widget:set_markup(markup.fontfg(theme.font, "#689dcd", mem_now.used .. "M " .. "/" .. mem_now.total .. "M"))
    end
})
...
local cpuicon = awful.widget.watch(
    "echo 'CPU: ' ", 60,
    function(widget, stdout)
        widget:set_markup(" " .. markup.fontfg(theme.font, "#99bfb8", stdout))
    end
)
local cpu = lain.widget.cpu({
    settings = function()
        widget:set_markup(markup.fontfg(theme.font, "#99bfb8", cpu_now.usage .. "%"))
    end
})
...
local temp = lain.widget.temp({
    settings = function()
        widget:set_markup(markup.fontfg(theme.font, "#99bfb8", coretemp_now .. "°C"))
    end
})
--]]
...
local bat = lain.widget.bat({
    settings = function()
        ...
        widget:set_markup(markup.fontfg(theme.font, "#689dcd", perc .. ""))
    end
})
...
theme.volume = lain.widget.alsa({
    settings = function()
        if volume_now.status == "off" then
            widget:set_markup(markup.fontfg(theme.font, "#99bfb8", "M"))
        -- elseif tonumber(volume_now.level) == 0 then
        --     volicon:set_image(theme.widget_vol_no)
        -- elseif tonumber(volume_now.level) <= 50 then
        --     volicon:set_image(theme.widget_vol_low)
        else
            widget:set_markup(markup.fontfg(theme.font, "#99bfb8", volume_now.level .. "%"))
        end
   ...
   s.quake = lain.util.quake({ app = "urxvt", height = 0.50, argname = "--name %s" })
	...
	spacing = -7,
        ...
        { -- Right widgets
            ...
            -- wibox.container.background(wibox.container.margin(wibox.widget { mainSep, layout = wibox.layout.align.horizontal }, dpi(bar_widget_sep_margin_left), dpi(bar_widget_sep_margin_right)), "#282c34"),
            wibox.container.background(wibox.container.margin(wibox.widget { brightnessicon, brightness, layout = wibox.layout.align.horizontal}, dpi(0), dpi(0), dpi(0), dpi(2), "#689dcd"), "#282c34"),
            -- arrow("#467b96", "#889fa7"),
            wibox.container.background(wibox.container.margin(wibox.widget { mainSep, layout = wibox.layout.align.horizontal }, dpi(bar_widget_sep_margin_left), dpi(bar_widget_sep_margin_right)), "#282c34"),
            wibox.container.background(wibox.container.margin(wibox.widget { cpuicon, cpu.widget, layout = wibox.layout.align.horizontal }, dpi(0), dpi(0), dpi(0), dpi(2), "#99bfb8"), "#282c34"),
            wibox.container.background(wibox.container.margin(wibox.widget { mainSep, layout = wibox.layout.align.horizontal }, dpi(bar_widget_sep_margin_left), dpi(bar_widget_sep_margin_right)),"#282c34"),
            wibox.container.background(wibox.container.margin(wibox.widget { baticon, bat.widget, layout = wibox.layout.align.horizontal }, dpi(0), dpi(0), dpi(0), dpi(2), "#689dcd"), "#282c34"),
            wibox.container.background(wibox.container.margin(wibox.widget { mainSep, layout = wibox.layout.align.horizontal }, dpi(bar_widget_sep_margin_left), dpi(bar_widget_sep_margin_right)),"#282c34"),
            wibox.container.background(wibox.container.margin(wibox.widget { tempicon, temp.widget, layout = wibox.layout.align.horizontal }, dpi(0), dpi(0), dpi(0), dpi(2), "#99bfb8"), "#282c34"),
            -- arrow("#497B96", "#889FA7"),
            -- wibox.container.background(wibox.container.margin(wibox.widget { weathericon, theme.weather.widget, layout = wibox.layout.align.horizontal }, dpi(3), dpi(3)), "#282c34"),
            wibox.container.background(wibox.container.margin(wibox.widget { mainSep, layout = wibox.layout.align.horizontal }, dpi(bar_widget_sep_margin_left), dpi(bar_widget_sep_margin_right)),"#282c34"),
            wibox.container.background(wibox.container.margin(wibox.widget { memicon, mem.widget, layout = wibox.layout.align.horizontal }, dpi(0), dpi(0), dpi(0), dpi(2), "#689dcd"), "#282c34"),
            wibox.container.background(wibox.container.margin(wibox.widget { mainSep, layout = wibox.layout.align.horizontal }, dpi(bar_widget_sep_margin_left), dpi(bar_widget_sep_margin_right)),"#282c34"),
            wibox.container.background(wibox.container.margin(wibox.widget { volicon, theme.volume, layout = wibox.layout.align.horizontal }, dpi(0), dpi(0), dpi(0), dpi(2), "#99bfb8"), "#282c34"),
            wibox.container.background(wibox.container.margin(wibox.widget { mainSep, layout = wibox.layout.align.horizontal }, dpi(bar_widget_sep_margin_left), dpi(bar_widget_sep_margin_right)),"#282c34"),
            wibox.container.background(wibox.container.margin(clock, dpi(0), dpi(0), dpi(0), dpi(2), "#689dcd"), "#282c34"),
            -- wibox.container.background(wibox.widget { mainSep, layout = wibox.layout.align.horizontal }, "#282c34"),
            --]]
            -- wibox.layout.margin(wibox.widget.systray(), 1, 1, 3, 3), --order of padding values is: left, right, top, bottom
            -- s.mylayoutbox,
